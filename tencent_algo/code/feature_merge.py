...
df_test = pd.read_csv(files_path + 'update_Btest_sample.dat',names=['item_id', 'ad_id','init_time','material_size',
...
df_static.bid_1.apply(trans_int)
df_total_log = pd.merge(df_total_log,df_static,on='ad_id',how='left')
df_total_log.period.fillna('-999',inplace=True)
df_total_log = df_total_log[df_total_log.period!='-999']
df_total_log.drop_duplicates(inplace=True)
print(df_total_log)
df_total_log.reset_index(drop=True)
...
for col in ['ad_account_id', 'goods_type', 'goods_id', 'ad_industry_id']:
    static_cols.remove(col)
df_test = pd.merge(df_test,df_static[static_cols].drop_duplicates('ad_id'),on='ad_id',how='left')
all_cols += ['bid_1','state']
print(df_test)
print('hhh')
